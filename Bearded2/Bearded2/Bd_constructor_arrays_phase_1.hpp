...
Plan_3.initialise(first_y,last_y+1-(first_y),first_y,last_y+1-(first_y),first_y,last_y+1-(first_y),0,1 +1-(0));

Plan_3.create(MemAllocator, Pr_PO_ytbs);

zero(Pr_PO_ytbs);

Plan_3.create(MemAllocator, sqrt_Pr_PO_ytbs);

zero(sqrt_Pr_PO_ytbs);

Plan_4.initialise(first_y,last_y+1-(first_y),0,1 +1-(0));

Plan_4.create(MemAllocator, inv_totfec_ys);

zero(inv_totfec_ys);

Plan_4.create(MemAllocator, N_ys);

zero(N_ys);

Plan_5.initialise(1,n_par +1-(1));

Plan_5.create(MemAllocator, temp_pars);
...
Plan_6.initialise(1,n_POP +1-(1));

Plan_6.create(MemAllocator, isamp_POP);

AdtArrayPlanActor::R_to_ADlib(MemAllocator, (char*)arg_isamp_POP, (char*)isamp_POP);

Plan_6.create(MemAllocator, jsamp_POP);
