...
EXPORT void RBd_destroy_handler(SEXP rInstance)
{
  ASSERT_TYPE_TAG(rInstance, DBearded2);
  
  RIBearded2* pContext = (RIBearded2*)R_ExternalPtrAddr(rInstance);
...
EXPORT SEXP RBd_destroy(SEXP args)
{
  SEXP rInstance;
  args = CDR(args); rInstance = CAR(args);
  RBd_destroy_handler(rInstance);
...
EXPORT SEXP RBd_create(SEXP args)
{
  SEXP Result = {0};
  RIBearded2* pContext = 0;
  ...
  if (pContext != 0)
  {
    MAKE_R_EXTERNAL_PTR(Result, pContext, RBd_destroy_handler, DBearded2);
  }
  else
  {
    Rf_error("ERROR: RBd_create() failed");
