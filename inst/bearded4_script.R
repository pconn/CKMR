...
for(k in 1:nyrs) {
    ...
    Alive.breed.F[k]=sum(indiv[,2]=="F" & as.numeric(indiv[,8])>firstBreed)
...
#fit_bd_sim <-function(Cenv,sim_data,par_start){
  ...
  amat = 7  #definitely need to generalize this; age at maturity is age at maturity here (note it was amat+1 for sims)
  ...
  isamp_HS = c(sim_data$isamp_MHSP,sim_data$isamp_PHSP)
