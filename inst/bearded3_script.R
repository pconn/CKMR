...
indiv <- makeFounders(pop = 2000, osr = c(0.5, 0.5),stocks=1,maxAge=n.ages,survCurv=Stable.age)
nyrs = 80
...
for(k in 1:nyrs) {
    ...
    Alive.breed.F[k]=sum(indiv[,2]=="M" & as.numeric(indiv[,8])>firstBreed)
...
N_y = getc("N_ys")
plot(Alive.breed.F[30:50],ylim=c(500,1100))
