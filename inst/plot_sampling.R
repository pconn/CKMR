...
Pred1 = Pred2 = Pred3 = expand.grid(Easting=c(1:10),Northing=c(1:10))
Pred1$Probability=0.01

strata_probs = rep(0,100)
for(i in 1:10){
  strata_probs[10*(i-1)+c(1:10)]=(i-1)*2/90+.2
}
Pred2$Probability=strata_probs/sum(strata_probs) #normalize

strata_probs = rep(0,100)
strata_probs[91:100]=0.4
strata_probs[81:90]=0.2
strata_probs[71:80]=0.1
Pred3$Probability=strata_probs/sum(strata_probs) #normalize
...
moderate_plot = ggplot(Pred2)+geom_raster(aes(x=Easting,y=Northing,fill=Probability))+
  ...
  scale_fill_continuous(limits=c(0,0.014))
moderate_plot
...
library(gridExtra)
library(lattice)
pdf("sampling_fig.pdf",width=4,height=8)
grid.arrange(random_plot,moderate_plot,extreme_plot,ncol=1)
dev.off()
