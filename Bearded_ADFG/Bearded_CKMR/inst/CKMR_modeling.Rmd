...
```{r,tmb,tidy='styler',cache=T,include=FALSE}
  ...
  save.image("TMB_workspace.RData")
  
  load("TMB_workspace.RData")
  Random=NULL
  library(TMB)
  getwd()
...
those that don't.  In particular, if the older animal is female, there is only a $\approx 50\%$ chance that GGPs will share mitochondrial DNA since the (unobserved) mother must also be female.  If the older animal is male, there is only a very small chance that the two are GGPs if they share mtDNA, and are more likely MHSPs.  For purposes of bearded seals, we'll start by grouping apparent half-sibling comparisons by the sex of the older individual (for same-age comparisons, the sex of an arbirtrary seal is chosen):

```{r,GGP_comps,tidy='styler',cache=T,include=FALSE}
  n_comp_m_HS_bibj = n_comp_f_HS_bibj = n_match_m_PHS_bibj= n_match_m_MHS_bibj = n_match_f_PHS_bibj=    
   n_match_f_MHS_bibj = = matrix(0,n_ages+n_yrs_data,n_ages+n_yrs_data) #years 41:(80+n_yrs_data)
