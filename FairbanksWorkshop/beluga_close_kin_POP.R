...
#attempting to estimate 1) # mature females, 2) # mature males, 3) sexual maturity age for males
#note (1) and (2) are assumed constant through time in this first run through
...
get_P_beluga <- function(Pars,P_Mother,P_Father,Maturity_age,n_yrs,t_start,t_end){
    ...
    for(ideath1 in max(ibirth1,t_start):t_end){ 
      for(ibirth2 in 1:n_yrs){
        if(ibirth2<=ideath1 & ibirth2>ibirth1 & ((ibirth2-ibirth1)<=n_ages))
          P_Mother[ibirth1,ideath1,ibirth2]=Maturity_age[ibirth2-ibirth1]/N_F
      }
    } 
  }
  ...
  for(ibirth1 in 1:n_yrs){  #dad
    for(ideath1 in max(ibirth1,t_start):t_end){ 
      for(ibirth2 in 1:n_yrs){
        if(ibirth2<=ideath1 & ibirth2>ibirth1 & ((ibirth2-ibirth1)<=n_ages))
          P_Father[ibirth1,ideath1,ibirth2]=Paternity_age[ibirth2-ibirth1]/N_M
      }
    } 
  }
...
Pars=c(log(4000),log(5000),logit(age_mat_male/n_ages))
...
Mort_age = rep(0.95,n_ages)
Mort_age[1]=0.85
Prop_age = rep(1,n_ages)
for(iage in 2:n_ages)Prop_age[iage]=Prop_age[iage-1]*Mort_age[iage-1]
Prop_age=Prop_age/sum(Prop_age)
...
#fit model!!
library(optimx)
CK_fit <- optimx(par=Pars,fn=beluga_neg_log_lik,hessian=TRUE,Negatives_Mother=Data_mom_no,Negatives_Father=Data_dad_no,Pairs_Mother=Data_mom_yes,Pairs_Father=Data_dad_yes,P_Mother=P_Mother,P_Father=P_Father,Maturity_age=Maturity_age,n_yrs=n_yrs,t_start=t_start,t_end=t_end)

#compute variance covariance matrix 
D=diag(3)*c(exp(CK_fit$p1[2]),exp(CK_fit$p2[2]),n_ages*exp(CK_fit$p3[2])/(1+exp(CK_fit$p3[2]))^2) #derivatives of transformations
