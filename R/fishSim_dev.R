...
#############################################
#' move_as(): markovian movement between breeding stocks.  this version allows variation by age and sex
#'
#' returns a pop-by-8 character matrix, defined in the makeFounders documentation. Dev note:
#' this version will still apply movement to dead individuals, but most users would probably
#' expect the dead to stay in the stock where they died. Update that.
#'
#' @param indiv Individual matrix, e.g. from makeFounders(). Can also be a non-founder matrix.
#' @param Move A list of lists giving an s-by-s transition matrix (describing the probability of moving from each stock to
#'                each other stock, with 'from' by row and 'to' by column) for each sex and age.  For instance,
#'                Move[[1]][[1]] gives the transition matrix for age 0 females 
#' @export

move_as <- function(indiv = makeFounders(), Move = MOve) {
  if(length(Move)!=2) warning("transition matrices must be provided for both sexes")
  if(nrow(Move[[1]][[1]]) != ncol(Move[[1]][[1]])) warning("movement matrix is not square")
  if(nrow(Move[[1]][[1]]) > length(unique(indiv[,7]))) warning("One or more stocks start empty")
  
  newStock <- matrix(data = NA, nrow = nrow(indiv))
  Which.alive = which(is.na(indiv[,6]))
  max.age = max(as.numeric(indiv[Which.alive,8]))
  Ages = as.numeric(indiv[,8])
  Strata = as.numeric(indiv[,7])
  Alive = is.na(indiv[,6])
  All.strata = c(1:nrow(Move[[1]][[1]]))
  if(length(Which.alive)!=nrow(indiv))newStock[which(!Alive)]=indiv[which(!Alive),7] #dead don't move
  for(csex in c("F","M")){
    for(iage in 1:max.age){
      for(istrata in 1:nrow(Move[[1]][[1]])){
        Which.sample = which(indiv[,2]==csex & Ages==iage & Strata==istrata & Alive==1) #identify all animals with same sex, age, strata profile for faster application of "sample"
        if(length(Which.sample)>0){
          newStock[Which.sample] = sample(All.strata,length(Which.sample),TRUE,prob=Move[[csex]][[iage]][istrata,])
        }
      }
    }
  }
  indiv[,7]=as.character(newStock)
  return(indiv)
}
